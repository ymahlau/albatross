...
class OvercookedEnv(object):
    ...
    def __init__(self, mdp_generator_fn, start_state_fn=None, horizon=MAX_HORIZON, mlam_params=NO_COUNTERS_PARAMS, info_level=0, num_mdp=1, initial_info={}):
        ...
        Potentially make changes based on this discussion
    ...
    def copy(self):
        # Add testing for checking that these util methods are up to date?
        return OvercookedEnv(
    ...
    def print_state_transition(self, a_t, r_t, env_info, fname=None, display_phi=False):
        ...
        """
        # turn this into a "formatting action probs" function and add action symbols too
    ...
    def reset(self, regen_mdp=True, outside_info={}):
        ...
        rewards_dict = {
            "cumulative_sparse_rewards_by_agent": np.array([0] * self.mdp.num_players, dtype=float),
            "cumulative_shaped_rewards_by_agent": np.array([0] * self.mdp.num_players, dtype=float)
        }
    ...
    def _prepare_info_dict(self, joint_agent_action_info, mdp_infos):
        ...
        # This can be further simplified by having all the mdp_infos copied over to the env_infos automatically
    ...
    def _update_game_stats(self, infos):
        ...
        self.game_stats["cumulative_shaped_rewards_by_agent"] += np.array(infos["shaped_reward_by_agent"], dtype=float)
    ...
    def get_rollouts(self, agent_pair, num_games, display=False, dir=None, final_state=False, display_phi=False,
        ...
        # should probably transfer check methods over to Env class
