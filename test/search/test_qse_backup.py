...
from src.search.config import AreaControlEvalConfig, FixedDepthConfig, SpecialExtractConfig, QSEBackupConfig
from src.search.initialization import get_search_from_config


class TestQSEBackup(unittest.TestCase):
    def test_qse_choke(self):
        gc = perform_choke_2_player(centered=False, fully_connected=False)
        env = BattleSnakeGame(gc)
        eval_func_cfg = AreaControlEvalConfig()
        backup_func_cfg = QSEBackupConfig(init_temperature=5)
