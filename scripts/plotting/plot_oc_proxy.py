...
def plot_both():
    ...
    base_name_fix = 'resp_proxy_fix_temps_1_1'
    temperatures = np.linspace(0, 10, 100)
    num_seeds = 5
    ...
    for idx, (prefix, full_name) in enumerate(name_dict.items()):
        ...
        result_list = []
        for seed in range(num_seeds):
            with open(save_path / f'{base_name_fix}_{prefix}_{seed}.pkl', 'rb') as f:
                cur_results = pickle.load(f)
            result_list.append(cur_results)
        result_arr = np.asarray(result_list)[:, :, 0, :]
        result_arr_resp_fix = np.transpose(result_arr, axes=(1, 0, 2)).reshape(len(temperatures), -1)
        ...
        plt.xlabel('Temperature', fontsize=fontsize)
        plt.tight_layout()
        plt.savefig(img_path / f'both_fix_resp_proxy_{prefix}.pdf', bbox_inches='tight', pad_inches=0.0)
